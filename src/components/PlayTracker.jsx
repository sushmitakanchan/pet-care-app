...
const PlayTracker = () => {
    const {dogInfo, walkProgressCounter, walkLabel, bathProgressCounter, bathLabel, feedProgressCounter, feedLabel, playProgressCounter, setPlayProgressCounter, playLabel, setPlayLabel} = useContext(PetContext)
    const navigate = useNavigate()
    const handlePlay = () =>{
        if(playProgressCounter>=2) return;
        setPlayProgressCounter(playProgressCounter+1)
    }
    useEffect(()=>{
        if(playProgressCounter===0){
            setPlayLabel(`Next playtime: Morning at ${dogInfo.play.morning}`)
        }else if(playProgressCounter===1){
            setPlayLabel(`Next playtime: Evening at ${dogInfo.play.evening}`)
        } else {
            setPlayLabel(`Playing done!`)
        }
    }, [playProgressCounter, setPlayLabel, dogInfo.play])
  return (
